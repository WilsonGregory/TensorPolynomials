...
def get_patterns(k,n):
    ...
    """
    patterns = []
    for i in range(1,min(k,n)+1): # available digits you can use, can't be 4 unique digits if n=3
        for p in partitions(k,i,1): # possible partitions of k items in i buckets, min=1 item per bucket
            if sorted(p, reverse=True) != p: # ensure p is in descending order because (1,3) and (3,1) are the same
                continue

            chars = it.chain.from_iterable([(char,)*char_count for char, char_count in enumerate(p)])
...
class PermInvariantTensor:
    ...
    def get(cls, k, n, symmetric_axes=()):
        ...
        if (k,n,symmetric_axes) not in cls.basis_dict:
            ...
            paired_tensors = [tensor_from_pattern(k,n,a) + tensor_from_pattern(k,n,b) for a,b in paired_patterns]
            cls.basis_dict[(k,n,symmetric_axes)] = jnp.stack(perm_invariant_tensors + paired_tensors)
